...
interface RegisteredTool extends CapturedTool {
	...
	parameters: {
		properties?: Record<
			string,
			{ maximum?: number; maxItems?: number; items?: { enum?: unknown[] } }
		>;
	};
}
...
describe("Forgejo tool activation", () => {
	...
	it("rejects all-at-once and oversized domain activation", async () => {
		...
		expect(loader.parameters.properties?.domains?.items?.enum).not.toContain(
			"all",
		);
	...
	it("lite mode swaps domains instead of accumulating them", async () => {
		...
		expect(result.content[0]?.text).toContain("Disabled Forgejo tools (lite mode): forgejo_pull, forgejo_review");
	...
	it("keeps lazy tools out of the system prompt and caps their model-visible output", () => {
		...
		expect(
			tools.get("forgejo_tools")?.parameters.properties?.domains?.items?.enum,
		).toContain("watch");
		const lazyNames = [
			"forgejo_actions",
			"forgejo_dashboard",
		...
		for (const name of lazyNames.filter(
			...
			expect(
				tools.get(name)?.parameters.properties?.max_bytes?.maximum,
				name,
			).toBe(128_000);
		}
	});
