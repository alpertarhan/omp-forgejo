...
function capabilities(
  overrides: Partial<ForgejoCapabilities["features"]> = {},
): ForgejoCapabilities {
  return {
    server: "work",
    version: "16.0.2",
...
function fakeRuntime(
  ...
  options: {
    cwd?: string;
    features?: Partial<ForgejoCapabilities["features"]>;
  } = {},
): ForgejoRuntime {
  ...
  return {
    ...
    capabilities: {
      get: () => capability,
      refreshAlias: vi.fn(async () => capability),
      refresh: vi.fn(async () => ({
        values: { work: capability },
        errors: {},
      })),
    },
    dashboard: {
...
describe("forgejo_actions mutation safety", () => {
  it("confirms exact dispatch inputs but keeps their values out of model-facing output", async () => {
    const request = vi.fn(async (path: string, options?: RequestOptions) => {
      expect(path).toBe(
        "repos/acme/app/actions/workflows/release.yml/dispatches",
      );
    ...
    expect(ui.select.mock.calls[0]?.[0]).toContain(
  ...
  it("refuses an unadvertised rerun before reading or mutating the run", async () => {
    ...
    const runtime = fakeRuntime(request, {
      features: { actionsRerun: "unavailable" },
    });
    const tool = captureActionsTool(runtime);
    const ui = confirmedContext();
  ...
  it("cancels an active run only after confirmation, revalidation, and reads back its state", async () => {
    let readCount = 0;
    const request = vi.fn(async (path: string, options?: RequestOptions) => {
      ...
      readCount += 1;
      return apiResult(actionRun(readCount < 3 ? "running" : "success"));
    });
    const runtime = fakeRuntime(request);
    const tool = captureActionsTool(runtime);
    const ui = confirmedContext();
  ...
  it("does not cancel a run that finishes while confirmation is open", async () => {
    let readCount = 0;
    const request = vi.fn(async (_path: string, options?: RequestOptions) => {
      if (options?.method === "POST") throw new Error("must not post");
      readCount += 1;
...
describe("forgejo_actions artifact download safety", () => {
  it("writes exact ZIP bytes atomically and refuses an implicit overwrite", async () => {
    ...
    const request = vi.fn(async (path: string, options?: RequestOptions) => {
      if (path.endsWith("/zip")) {
        ...
        return apiResult(new Blob([archive]).stream());
      }
      return apiResult({
        id: 6,
        name: "coverage",
    ...
    try {
      ...
      expect([...(await readFile(join(cwd, "coverage.zip")))]).toEqual([
        ...archive,
      ]);
  ...
  it("rejects a destination outside the workspace before downloading bytes", async () => {
    ...
    const request = vi.fn(async (path: string) => {
      if (path.endsWith("/zip")) {
        downloadCalls += 1;
        return apiResult(new Blob([new Uint8Array([1])]).stream());
      }
      return apiResult({
        id: 6,
        name: "coverage",
