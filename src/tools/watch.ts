...
export function registerWatchTool(
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      action: Type.Enum(["start", "list", "stop"] as const),
      ref: Type.Optional(Type.String()),
      events: Type.Optional(
        Type.Array(Type.Enum(WATCH_EVENTS), {
          minItems: 1,
          uniqueItems: true,
      ...
      target: Type.Optional(
        Type.Enum(["review_requests", "notifications"] as const, {
          description: "Cross-server source instead of ref",
        }),
      ),
      since: Type.Optional(Type.String({ format: "date-time" })),
      // Schema defaults are omitted on purpose: omp's validation layer
      // materializes them into params, which would trip rejectFields on
      // the start paths that forbid these fields. execute() applies the
      // fallbacks itself.
      ...
      attention: Type.Optional(
        Type.Enum(["turn", "context"] as const),
      ),
