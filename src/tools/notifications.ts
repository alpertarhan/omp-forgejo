...
function apiSegments(baseUrl: string, value: string, label: string): string[] {
  ...
  if (target.origin !== base.origin)
    throw new Error(
      `${label} points outside configured Forgejo origin ${base.origin}`,
    );
  if (target.username || target.password || target.search || target.hash) {
    throw new Error(
      `${label} contains unsupported credentials, query, or fragment`,
    );
  }
  const prefix = `${base.pathname.replace(/\/+$/, "")}/api/v1/`;
  if (!target.pathname.startsWith(prefix))
    throw new Error(`${label} is outside the configured Forgejo API root`);
...
function notificationTarget(
  ...
  if (subjectType !== "issue" && subjectType !== "pull") {
    throw new Error(
      `notification ${notification.id} does not reference an issue or pull request`,
    );
  }
  if (!notification.subject.url)
    throw new Error(`notification ${notification.id} has no subject API URL`);
  ...
  if (
    ...
    throw new Error(
      `notification ${notification.id} subject URL is not an expected ${expectedRoute} route`,
    );
  }
  const index = Number(indexValue);
  if (!Number.isInteger(index) || index < 1)
    throw new Error(
      `notification ${notification.id} subject URL has an invalid index`,
    );
  if (`${owner}/${repo}` !== notification.repository.full_name) {
    throw new Error(
      `notification ${notification.id} subject URL does not match ${notification.repository.full_name}`,
    );
  }
...
function latestCommentPath(
  ...
  if (
    ...
    throw new Error(
      `notification ${notification.id} latest comment URL is not an expected repository issue-comment route`,
    );
  }
  return apiPath(
    "repos",
...
function notificationReference(
  server: string,
  notification: ForgejoNotification,
): string {
  ...
  for (const candidate of candidates) {
    ...
    if (match?.[1])
      return `${server}:${fullName}${type === "pull" ? "!" : "#"}${match[1]}`;
  }
...
function formatNotification(
  ...
  const flags = [
    ...
    notification.pinned ? "pinned" : undefined,
  ].filter(Boolean);
  const lines = [
  ...
  if (notification.subject.latest_comment_url)
    lines.push(
      `Latest comment API URL: ${notification.subject.latest_comment_url}`,
    );
  if (notification.subject.latest_comment_html_url)
    lines.push(
      `Latest comment web URL: ${notification.subject.latest_comment_html_url}`,
    );
  if (notification.subject.html_url)
    lines.push(`Web URL: ${notification.subject.html_url}`);
  return lines.join("\n");
}
...
export function registerNotificationTool(
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
          ...
          aliases.map(async (alias) => {
            try {
              const response = await runtime
                .client(alias)
                .request<ForgejoNotification[]>("notifications", {
                  ...requestOptions,
                  query: {
                    "status-types": ["unread"],
            ...
            result.error
              ...
              : [
                  ...
                  ...result.items.map((notification) =>
                    formatNotification(result.server, notification),
                  ),
                ],
          )
          .join("\n\n");
        const bounded = boundModelText(
          text || "No unread Forgejo notifications",
          params.max_bytes ?? DEFAULT_MODEL_OUTPUT_BYTES,
        );
        const errors = Object.fromEntries(
          outcomes.flatMap((result) =>
            result.error ? [[result.server, result.error]] : [],
          ),
        );
      ...
      if (!alias)
        throw new Error(
          "server is required when multiple Forgejo servers are configured",
        );
      ...
      if (params.action === "get") {
        ...
        if (params.resolve_latest) {
          ...
          const commentPath = latestCommentPath(
            client.config.baseUrl,
            response.data,
            target,
          );
          if (commentPath) {
            const comment = await client.request<ForgejoComment>(
              commentPath,
              requestOptions,
            );
            assertForgejoCommentTarget(
