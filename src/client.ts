...
export const USER_AGENT = `omp-forgejo/${PACKAGE_VERSION ?? "dev"}`;
...
// Version thresholds for the Actions API family, used only when Swagger
// discovery is unreachable. Sources: Forgejo release notes — workflow
// dispatch shipped in v8.0 (July 2024), run list/get in v12.0 (July 2025),
// and job/workflow logs, artifacts, and run cancellation in v16.0
// (July 2026). Rerun landed after v16 and keeps the "unknown" fallback.
const ACTION_DISPATCH_MIN_VERSION = 8;
const ACTION_RUNS_MIN_VERSION = 12;
const ACTION_CANCEL_AND_ARTIFACTS_MIN_VERSION = 16;
...
function versionAvailability(
  ...
  return Number(match[1]) >= minimumMajor ? "available" : "unavailable";
}
...
export class ForgejoClient {
  ...
  async request<T>(
    ...
    // Acquire credentials first: the request timeout must not tick while a
    // slow credential source (e.g. an fgj subprocess) is resolving.
    let token = await this.credentials.getToken(options.signal);
    ...
    while (true) {
      ...
      if (
        response.status === 401 &&
        this.credentials.kind === "fgj" &&
        !authRetried
      ) {
        // A cached fgj token may have been revoked or rotated server-side.
        // Drop it, resolve a fresh token, and retry exactly once.
        await response.body?.cancel().catch(() => undefined);
        authRetried = true;
        this.credentials.clear();
        token = await this.credentials.getToken(options.signal);
        redirects = 0;
        continue;
      }
    ...
    return {
      ...
      features: {
        ...
        actionsRuns: actionPathAvailability(
          paths,
          ACTION_RUNS_PATH,
          versionAvailability(version, ACTION_RUNS_MIN_VERSION),
        ),
        actionsDispatch: actionPathAvailability(
          paths,
          ACTION_DISPATCH_PATH,
          versionAvailability(version, ACTION_DISPATCH_MIN_VERSION),
        ),
        actionsCancel: actionPathAvailability(
          paths,
          ACTION_CANCEL_PATH,
          versionAvailability(version, ACTION_CANCEL_AND_ARTIFACTS_MIN_VERSION),
        ),
        actionsRerun: actionPathAvailability(paths, ACTION_RERUN_PATH),
        actionsArtifacts: actionPathAvailability(
          paths,
          ACTION_ARTIFACTS_PATH,
          versionAvailability(version, ACTION_CANCEL_AND_ARTIFACTS_MIN_VERSION),
        ),
      },
    };
