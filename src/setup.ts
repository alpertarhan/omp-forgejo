...
async function customDashboard(
	...
	const widgetOn =
		"Show the compact dashboard widget in matching Forgejo repositories";
...
function dashboardSummary(dashboard: DashboardConfig): string {
	return [
		dashboard.enabled ? "widget on in Forgejo repositories" : "widget off",
...
async function configureDashboard(
	...
	const recommended =
		"Recommended — widget, all servers, important popups, full details";
	const quiet = "Quiet — current repository, no popups, 5-minute refresh";
	const privateMode =
		"Private — all-server counts with hidden repository details";
