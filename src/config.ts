...
/**
 * Called by the extension entry once the active omp agent directory is
 * known. Keeps config.ts free of pi-coding-agent imports (which would drag
 * optional dependencies into the test graph) while letting the running
 * host decide where config lives.
 */
 ...
 *
 * The global configuration lives in the active agent directory
 * (~/.omp/agent by default, ~/.omp/profiles/<name>/agent under `omp
 * --profile <name>`, relocated by PI_CODING_AGENT_DIR which omp honors).
 * The project configuration lives in <cwd>/.omp/forgejo.json and is only
 * read for projects omp marks trusted.
 */
export function configPaths(
  ...
  const global = resolve(
    hostContext?.agentDir ?? resolve(homedir(), ".omp", "agent"),
    "forgejo.json",
  );
  return {
    global: env.OMP_FORGEJO_CONFIG ? resolve(env.OMP_FORGEJO_CONFIG) : global,
    project: resolve(cwd, ".omp", "forgejo.json"),
  };
}
